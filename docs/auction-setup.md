...
Buyer contact information is AES-256-GCM encrypted before storage. The encrypted `.enc` blob may live in the site's existing public-access Blob store for compatibility, but its contents are ciphertext and the decryption key remains server-side in the admin secret. The separate public claim slot contains no purchaser PII.
