...
The live source of truth is Vercel Blob, reusing the Blob store already configured for website image uploads. The reservation/order itself is one deterministic encrypted Blob record for the listing and is written with overwrite disabled. This single atomic write simultaneously captures the purchaser/order data and removes the unique item from availability, so two shoppers cannot successfully reserve the same listing.

Buyer contact information is AES-256-GCM encrypted before storage. The reservation Blob contains ciphertext rather than readable purchaser data; its pathname is derived only from the internal marketplace item ID and contains no purchaser PII or order identifier. The decryption key remains server-side in the admin secret. Reserved inventory remains unavailable while payment is pending; failed/abandoned reservations must be released administratively before that item can be offered again.
