...
Each inventory unit has a deterministic claim slot written with overwrite disabled. The first successful write owns that slot, preventing two shoppers from claiming the same unit. Buyer contact information is AES-256-GCM encrypted before being stored; the public claim record contains no purchaser PII.
