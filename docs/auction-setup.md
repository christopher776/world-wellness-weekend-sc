...
## Inventory and order record
Each marketplace listing represents one unique item, experience, certificate or package. If a partner provides multiple identical units, create a separate listing for each unit. Listings are created at `/admin/marketplace` with photo, description, provider, retail value, launch/end time, fulfillment, terms, tax category, featured and published status. A photo and an explicit tax category are required before publishing.

The live source of truth is Vercel Blob, reusing the Blob store already configured for website image uploads. The reservation/order itself is one deterministic encrypted Blob record for the listing and is written with overwrite disabled. This single atomic write simultaneously captures the purchaser/order data and removes the unique item from availability, so two shoppers cannot successfully reserve the same listing and there is no secondary-write consistency gap.

Buyer contact information is AES-256-GCM encrypted before storage. The reservation/order Blob contains ciphertext; its pathname is derived only from the internal marketplace item ID and contains no purchaser PII or order identifier. The decryption key remains server-side in the admin secret. Reserved inventory remains unavailable while payment is pending; failed/abandoned reservations must be released administratively before that item can be offered again.

## Operational spreadsheet
The workbook `SC Wellness Make It Mine Marketplace` contains `MarketplaceItems`, `MarketplaceCustomers`, `MarketplaceOrders`, and `Compliance`. The site attempts a best-effort mirror of listing updates into `MarketplaceItems` through the existing CMS webhook. The encrypted reservation record is the authoritative order/inventory record; the workbook is the operational reporting workspace.
