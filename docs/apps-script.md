...
function doPost(e) {
  ...
  if (body.action === "upsert") {
    ...
    for (var i = 1; i < data.length; i++) {
      if (idCol >= 0 && data[i][idCol] === id) { rowIndex = i + 1; break; }
    }
  ...
  sheet.appendRow([new Date(),body.type||"",body.name||"",body.email||"",body.phone||"",body.organization||"",body.interest||"",body.message||"",body.classInterests||"",body.experienceLevel||"",body.accessibilityNotes||""]);
...
function formatCellValue(value, header) {
  ...
  if (h.indexOf("time") !== -1) return Utilities.formatDate(value, EVENT_TIME_ZONE, "h:mm a");
  if (h.indexOf("date") !== -1) return Utilities.formatDate(value, EVENT_TIME_ZONE, "yyyy-MM-dd");
...
function doGet(e) {
  if (e.parameter.action === "list") {
    ...
    var data = sheet.getDataRange().getValues();
    var headers = data[0];
    var rows = data.slice(1).map(function (r) { var obj={}; headers.forEach(function(h,i){obj[h]=formatCellValue(r[i],h);}); return obj; });
...
## MarketplaceSales sheet header row

Create a tab named exactly **MarketplaceSales**. This is the operational sales ledger used by the marketplace reservation, Authorize.Net payment verification, release and refund workflows. Use this header row:

`ID | OrderID | ItemID | ItemTitle | PurchaserName | Email | Phone | RetailValue | SalePrice | Status | PaymentStatus | TransactionID | ClaimedAt | PaidAt | CheckoutGenerated | PaymentSource | ReleaseReason | ReleasedAt | RefundTransactionID | RefundReason | RefundedAt | Relisted | UpdatedAt`

The order ID is the stable row key, so the same row progresses from RESERVED to PAID, RELEASED or REFUNDED instead of creating disconnected records.
...
| `BLOB_READ_WRITE_TOKEN` | Auto-added by Vercel when you connect a Blob store to this project |
