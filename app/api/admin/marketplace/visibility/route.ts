...
import { fetchMarketplaceItems, saveMarketplaceItem } from "@/lib/marketplace";
export const runtime = "nodejs";
export async function POST(request: Request) {
  let body: { itemId?: string; published?: boolean };
  try {
  ...
  try {
    ...
    if (!current)
      return NextResponse.json(
        { ok: false, error: "Marketplace item not found." },
        { status: 404 },
      );
    const item = {
      ...current,
    ...
    await upsertRow("MarketplaceItems", {
      ...
      Inventory:
        item.PricingMode === "FIXED"
          ? `${item.Quantity} units at fixed price`
