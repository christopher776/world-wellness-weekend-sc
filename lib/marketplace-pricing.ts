export const DEFAULT_MARKETPLACE_FLOOR_PERCENT = 15;
...
function floorPercent(item: MarketplacePricedItem) {
  const value = Number(item.FloorPercent);
  return Number.isFinite(value) && value > 0 && value < 100
    ? value
    : DEFAULT_MARKETPLACE_FLOOR_PERCENT;
}
function easternOffsetForLocal(normalized: string) {
  const datePart = normalized.slice(0, 10);
  const year = Number(datePart.slice(0, 4));
  if (!year) return "-04:00";
  ...
  if (datePart > dstStart && datePart < dstEnd) return "-04:00";
  if (datePart < dstStart || datePart > dstEnd) return "-05:00";
  const hour = Number(normalized.slice(11, 13) || 0);
  if (datePart === dstStart) return hour >= 3 ? "-04:00" : "-05:00";
  return hour < 2 ? "-04:00" : "-05:00";
}
export function parseCharlestonDateTime(value?: string | null) {
  ...
  const date = new Date(
    hasZone
      ? normalized
      : `${normalized}:00${easternOffsetForLocal(normalized)}`,
  );
...
export function marketplacePrice(
  ...
  if (item.PricingMode === "FIXED")
    return {
      price: retail,
      percentBelowRetail: 0,
      progress: 0,
      started: true,
      ended: false,
      floorPercent: 100,
    };
  const floorPct = floorPercent(item);
  const floor = retail * (floorPct / 100);
  const start = parseCharlestonDateTime(item.LaunchAt);
  const end = parseCharlestonDateTime(item.EndAt);
  if (!retail)
    return {
  ...
  if (now >= end)
    return {
      price: floor,
      percentBelowRetail: 100 - floorPct,
      progress: 1,
      started: true,
      ended: true,
      floorPercent: floorPct,
