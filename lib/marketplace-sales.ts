...
export async function syncMarketplaceSale(order:MarketplaceOrderPayload,state:MarketplaceSaleState,extra:Record<string,string>={}){
 const row:Record<string,string>={
  ...
  ItemPrice:(order.subtotal??order.price).toFixed(2),
  SalesTax:(order.salesTax??0).toFixed(2),
  SalesTaxRate:String(order.salesTaxRate??0),
  TotalCharged:order.price.toFixed(2),
  Status:state,
